...
#define MP_ASSERT_RETURN(condition, message, returnValue) if (!(condition)) return returnValue
#else
#define MP_ASSERT(condition, message) \
    do { \
        if (!(condition)) { \
            memory_pool::ErrorHandler::getInstance().reportError( \
                memory_pool::ErrorSeverity::Error, \
                std::string("Assertion failed: ") + (message) + \
                " [" + __FILE__ + ":" + std::to_string(__LINE__) + "]" \
            ); \
            memory_pool::ErrorHandler::getInstance().throwIfEnabled( \
                std::string("Assertion failed: ") + (message) \
            ); \
        } \
    } while(0)
