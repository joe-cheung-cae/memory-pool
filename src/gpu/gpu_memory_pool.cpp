...
void GPUMemoryPool::initialize() {
    ...
    if (config.allocatorType == AllocatorType::FixedSize) {
        allocator = std::make_unique<CudaFixedSizeAllocator>(
            config.blockSize, config.initialSize / config.blockSize, deviceId,
...
void* GPUMemoryPool::allocate(size_t size) { return allocateInternal(size, AllocFlags::None); }

void* GPUMemoryPool::allocate(size_t size, AllocFlags flags) { return allocateInternal(size, flags); }
...
const MemoryStats& GPUMemoryPool::getStats() const { return stats; }

MemoryType GPUMemoryPool::getMemoryType() const { return MemoryType::GPU; }

std::string GPUMemoryPool::getName() const { return name; }

const PoolConfig& GPUMemoryPool::getConfig() const { return config; }
...
int GPUMemoryPool::getDevice() const { return deviceId; }
...
cudaStream_t GPUMemoryPool::getStream() const { return stream; }
