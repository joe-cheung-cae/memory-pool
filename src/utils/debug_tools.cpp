#include "../../include/memory_pool/utils/debug_tools.hpp"
...
void MemoryLeakDetector::trackDeallocation(void* ptr, const std::string& poolName) {
    ...
    if (it != allocations.end()) {
        ...
        if (it->second.poolName != poolName) {
            reportWarning("Memory allocated in pool '" + it->second.poolName + 
                         "' but deallocated in pool '" + poolName + "'");
        }
    ...
    for (const auto& pair : trackedAllocations) {
        if (!checkBoundaries(pair.first, pair.second)) {
            ...
            reportError(ErrorSeverity::Error, 
                       "Memory boundary violation detected at " + 
...
bool BoundaryChecker::isEnabled() const {
    return enabled;
}
...
bool isDebuggingEnabled() {
    return MemoryLeakDetector::getInstance().isEnabled() ||
           BoundaryChecker::getInstance().isEnabled() ||
