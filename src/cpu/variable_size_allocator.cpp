...
VariableSizeAllocator::VariableSizeAllocator(size_t initialSize, size_t alignment)
    : alignment(alignment), minBlockSize(align_size(sizeof(BlockHeader) + 1, alignment)), totalSize(0), usedSize(0) {
    ...
    if (block == nullptr) {
        ...
        if (block == nullptr) {
            reportError(ErrorSeverity::Error,
                        "VariableSizeAllocator: Out of memory, failed to allocate " + std::to_string(size) + " bytes");
            return nullptr;
        }
    }
...
size_t VariableSizeAllocator::getTotalSize() const { return totalSize; }

size_t VariableSizeAllocator::getUsedSize() const { return usedSize; }

size_t VariableSizeAllocator::getFreeSize() const { return totalSize - usedSize; }
