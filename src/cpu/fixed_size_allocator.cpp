...
void* FixedSizeAllocator::allocate(size_t size) {
    ...
    if (size > blockSize) {
        reportError(ErrorSeverity::Warning, 
            "FixedSizeAllocator: Requested size " + std::to_string(size) + 
...
size_t FixedSizeAllocator::getCapacity() const {
    return totalBlocks;
}

size_t FixedSizeAllocator::getFreeBlocks() const {
    return totalBlocks - usedBlocks;
}

size_t FixedSizeAllocator::getUsedBlocks() const {
    return usedBlocks;
}
...
void FixedSizeAllocator::allocateChunk(size_t blockCount) {
    ...
    if (memory == nullptr) {
        reportError(ErrorSeverity::Error, 
            "FixedSizeAllocator: Failed to allocate memory for chunk");
        return;
    }
